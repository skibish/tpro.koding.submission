<?
	...
	class Oxygen_SQL_Builder extends Oxygen_Object {
		...
		public static function buildOrder($order) {
			return ($order === false)? $order : implode(", ", $order);
		}
		...
		public function buildSql($meta, $intent, $onlyCount = false) {
            ...
            if ($intent === 'select') {
                ...
                return 'insert into ' 
                	. self::buildDomain($meta['from'])
                    . ' set '
                    . self::buildSet($meta['set'])
                ;
            }
		...
		public function addOrderBy($meta, $order, $stable = false) {
			//die(print_r($order));
			if(is_string($order)) $order = array($order);
			if($meta['order'] === false){
				$meta['order'] = $order;
			}else{
				$meta['order'] = array_merge($meta['order'], $order);
			}
			return $meta;
		}

		public function replaceOrderBy($meta, $order, $stable = false) {
			//die(print_r($order));
			if(is_string($order)) $order = array($order);
			$meta['order'] = $order;
			return $meta;
