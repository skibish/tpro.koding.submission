...
function getMore(cb) {
    ...
    waitMore = true;
    $loader.show();
    $this.remote('getMore',{limit:$this.data('limit'), filter:$this.data('filter')},function(err,data){
        var count = 0;
        if(err) {
            o.flash(err,'error');
        } else {
            $this.data('limit', $this.data('limit')+data.count);
            count = data.count;
